...
function setup() {
  ...
  slider = createSlider(-1.08, 1.08, 0.08);
  createP('');
...
function launchProgram() {
  ...
  for(let i = 0; i < layerCount; i++) {
    bgLayers[i] = new Layer(colourFrom, colourTo, (incrementCol * i),
                            weight, i*1.9, randomGaussian(xCoord, 20),
...
function keyPressed(){
    ...
    if(key == '1'){ // RED / PURPLE
      ...
      colourBG = color(340, 90, 80);
      colourFrom = color(185, 90, 80);
      colourTo = color(155, 90, 80);
      colourHighlight = color(290, 65, 50, 5);
      colourHighlight2 = color(300, 65, 50, 5);
      launchProgram();
    } else if(key == '7'){ // BLUE / PINK
      colourBG = color(180, 90, 80);
